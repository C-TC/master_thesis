    ...
    def backward_gpu(self, A_mapping_blocks: Tuple, grad_out: np.ndarray):
        ...
        for k in range(0, self.A_shape[1], self.tau):
            ...
            for i in range(0, self.A_shape[0], self.tau):
                ...
                M_T_block_dev = cp.sparse.csr_matrix((cp.asarray(M_block_data), cp.asarray(A_block.indices), cp.asarray(A_block.indptr)),
        ...
        for k in range(0, self.A_shape[1], self.tau):
            ...
            for i in range(0, self.A_shape[0], self.tau):
                A_block = A_blocks[i // self.tau][k // self.tau]
                dC_block_data = dC_data_blocks[i // self.tau][k // self.tau]
                dC_T_block_dev = cp.sparse.csr_matrix((cp.asarray(dC_block_data), cp.asarray(A_block.indices), cp.asarray(A_block.indptr)),
                                                      shape=A_block.shape).T
