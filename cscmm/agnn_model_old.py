    ...
    def backward_gpu(self, A_mapping_blocks: Tuple, grad_out: np.ndarray):
        ...
        for k in range(0, self.A_shape[1], self.tau):
            ...
            for i in range(0, self.A_shape[0], self.tau):
                ...
                Q_T_block_dev = cp.sparse.csr_matrix((cp.asarray(Q_block_data), cp.asarray(A_block.indices), cp.asarray(A_block.indptr)),
        ...
        for k in range(0, self.A_shape[1], self.tau):
            ...
            for i in range(0, self.A_shape[0], self.tau):
                ...
                dD_T_block_dev = cp.sparse.csr_matrix(
                    (cp.asarray(dD_block_data), cp.asarray(A_block.indices), cp.asarray(A_block.indptr)),
                    shape=A_block.shape).T
