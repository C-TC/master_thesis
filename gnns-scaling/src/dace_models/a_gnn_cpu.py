...
def a_gnn_npsp2(A: sparse.csr_matrix,
                  H: np.ndarray,
                  W: np.ndarray) -> np.ndarray:
...
if __name__ == '__main__':
    ...
    sdfg, sdfgc = (None, ) * 2
    if rank == 0:
        # sdfg = a_gnn_dace.to_sdfg(simplify=True)
        sdfg = a_gnn_dace_loop.to_sdfg(simplify=True)
    ...
    runtimes = timeit.repeat(
        """out[:] = func(A_rowptr=A_rowptr, A_rowidx=A_rowidx, A_colidx=A_colidx, A_data=A_data, H1=H, W1=W, W2=W2,
    ...
    if rank == 0:
        ...
        write_time(str(datetime.now()), "a_gnn", "dace_cpu", size, weak_scaling[size], runtimes, file_name, field_names, append=True)

    # # # ref = a_gnn_npsp(A, H, W)
    # ref = a_gnn_npsp_loop(A, H, W, W2, num_layers)
    # lref = ref[x*tx:(x+1)*tx, :]
    # # # ref2 = a_gnn_npsp2(A, H, W)
