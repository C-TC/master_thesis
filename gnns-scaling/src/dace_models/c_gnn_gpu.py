...
""" Explicit distributed c_gnn attention sample programs. """
import csv
import cupy
import dace
import numpy as np
...
@dace.program
def c_gnn_dace(A_rowptr: dace.int32[LArows+1],
...
@dace.program
def c_gnn_dace_loop(A_rowptr: dace.int32[LArows+1],
                   A_rowidx: dace.int32[LAnnz],
                   A_colidx: dace.int32[LAnnz],
                   A_data: dctype[LAnnz],
                   H2: dctype[LAcols, LHcols],
...
@dace.program
def c_gnn_dace_loop_compute(A_rowptr: dace.int32[LArows+1],
                           A_rowidx: dace.int32[LAnnz],
                           A_colidx: dace.int32[LAnnz],
                           A_data: dctype[LAnnz],
                           H2: dctype[LAcols, LHcols],
...
def c_gnn_npsp(A: sparse.csr_matrix,
                 H: np.ndarray,
...
def c_gnn_npsp_loop(A: sparse.csr_matrix,
                      H: np.ndarray,
                      W1: np.ndarray,
                      W2: np.ndarray,
...
if __name__ == '__main__':
    ...
    sdfg, sdfgc = (None, ) * 2
    if rank == 0:
        sdfg = auto_gpu(c_gnn_dace_loop)
        sdfgc = auto_gpu(c_gnn_dace_loop_compute)
    ...
    if rank == 0:
        ...
        write_time(str(datetime.now()), "c_gnn", "dace_gpu", size, weak_scaling[size], runtimes, file_name, field_names, append=True)
        ...
        write_time(str(datetime.now()), "c_gnn_compute", "dace_gpu", size, weak_scaling[size], runtimes, file_name, field_names, append=True)

    # ref = c_gnn_npsp_loop(A, H, W, W2, num_layers)
