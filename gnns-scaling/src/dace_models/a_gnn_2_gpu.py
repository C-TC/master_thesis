...
""" Explicit distributed a_gnn attention sample programs. """
import csv
import cupy
import dace
import numpy as np
...
@dace.program
def a_gnn_dace(A_rowptr: dace.int32[LArows+1],
...
@dace.program
def a_gnn_dace_loop(A_rowptr: dace.int32[LArows+1],
                      A_rowidx: dace.int32[LAnnz],
                      A_colidx: dace.int32[LAnnz],
                      A_data: dctype[LAnnz],
                      H1: dctype[LArows, LHcols],
    ...
    # row degree
    H_row_degree = np.zeros(LArows)
    for j in dace.map[0:LArows]:
        H_row_degree[j] += np.sqrt(np.sum(np.square(H1[j,:])))

    # column degree
    H_col_degree = np.zeros(LAcols)
    for j in dace.map[0:LAcols]:
        H_col_degree[j] += np.sqrt(np.sum(np.square(H2[j, :])))
    ...
    # divide each nonzero element by degree
    for i in dace.map[0:LAnnz]:
        row = A_rowidx[i]
        col = A_colidx[i]
        val = (H_row_degree[row]*H_col_degree[col])
    ...
    for i in range(num_layers):
        ...
        H_row_degree = np.zeros(LArows)
        for j in dace.map[0:LArows]:
            H_row_degree[j] += np.sqrt(np.sum(np.square(H1_new[j,:])))

        # column degree
        H_col_degree = np.zeros(LAcols)
        for j in dace.map[0:LAcols]:
            H_col_degree[j] += np.sqrt(np.sum(np.square(H2_new[j, :])))
...
@dace.program
def a_gnn_dace_loop_compute(A_rowptr: dace.int32[LArows+1],
                              A_rowidx: dace.int32[LAnnz],
                              A_colidx: dace.int32[LAnnz],
                              A_data: dctype[LAnnz],
                              H1: dctype[LArows, LHcols],
...
def a_gnn_npsp(A: sparse.csr_matrix,
                 H: np.ndarray,
...
def a_gnn_npsp_loop(A: sparse.csr_matrix,
                      H: np.ndarray,
                      W1: np.ndarray,
                      W2: np.ndarray,
...
if __name__ == '__main__':
    ...
    sdfg, sdfgc = (None, ) * 2
    if rank == 0:
        sdfg = auto_gpu(a_gnn_dace_loop)
        sdfgc = auto_gpu(a_gnn_dace_loop_compute)
    ...
    if rank == 0:
        ...
        write_time(str(datetime.now()), "a_gnn", "dace_gpu", size, weak_scaling[size], runtimes, file_name, field_names, append=True)
        ...
        write_time(str(datetime.now()), "a_gnn_compute", "dace_gpu", size, weak_scaling[size], runtimes, file_name, field_names, append=True)

    # ref = a_gnn_npsp_loop(A, H, W, W2, num_layers)
