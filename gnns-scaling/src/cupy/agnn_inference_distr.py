...
def generate_blocks_inference(A: sparse.csr_matrix, H0_cols: int,
    ...
    """
    # Layer computation is σ((A * ((H @ H^T) / (||H|| @ ||H||^T)) @ (H @ W))
    # A: NI x NK (sparse)
    # H: NK x NJ (dense)
    # W: NJ x NL (dense)
    ...
    # ||H_1||: T x 1 (dense)
    # ||H_2||: T x 1 (dense)
    # A * ((H_1 @ H_2^T) / (||H_1|| @ ||H_2||^T)): T x T (sparse)
    # W: max_cols x max_cols (dense)
    # H_2 @ W: T x max_cols (dense)
    # (A * ((H_1 @ H_2^T) / (||H_1|| @ ||H_2||^T))) @ HW: T x max_cols (dense)
    ...
    # ||H_1||: dtype*T
    # ||H_2||: dtype*T
    # A * ((H_1 @ H_2^T) / (||H_1|| @ ||H_2||^T)): density*dtype*T^2 (data) + density*4*T^2 (indices) + 4*T (indptr)
    # H_2 @ W: dtype*T*max_cols
    # (A * ((H_1 @ H_2^T) / (||H_1|| @ ||H_2||^T))) @ HW: dtype*T*max_cols
    # out: dtype*T*max_cols
    # Total: T^2*(2*density*dtype + density*8) + T*(8 + 2*dtype + 5*dtype*max_cols) + max_cols^2*dtype
    ...
    beta = 8 + 2 * dtype + 5 * dtype * max_cols
...
def layer_cpu(A: sparse.csr_matrix, H_tile_1: np.ndarray, H_tile_2: np.ndarray, W: np.ndarray,
    ...
    """
    # row-wise computation of the L2 norm
    H_tile_1_norm = np.linalg.norm(H_tile_1, 2, axis = 1)
    H_tile_1_norm = H_tile_1_norm.reshape(H_tile_1_norm.shape[0], 1)
    H_tile_2_norm = np.linalg.norm(H_tile_2, 2, axis = 1)
    H_tile_2_norm = H_tile_2_norm.reshape(H_tile_2_norm.shape[0], 1)
    if H_tile_2.shape[1] > W.shape[1]:
        tmp = A.multiply(np.divide(H_tile_1 @ H_tile_2.T, H_tile_1_norm @ H_tile_2_norm.T)) @ (H_tile_2 @ W)
    else:
        tmp = (A.multiply(np.divide(H_tile_1 @ H_tile_2.T, H_tile_1_norm @ H_tile_2_norm.T)) @ H_tile_2) @ W
...
def layer_gpu(A: sparse.csr_matrix, A_blocks: List[sparse.csr_matrix], H_tile_1: np.ndarray, H_tile_2: np.ndarray,
              W: np.ndarray, tau: int, reduce_comm: MPI.Cartcomm) -> np.ndarray:
    ...
    for i in range(0, A.shape[0], tau):
        ...
        H_tile_1_tile = cp.asarray(H_tile_1[i:min(i + tau, H_tile_1.shape[0]), :])
        H_tile_1_tile_norm = np.linalg.norm(H_tile_1_tile, 2, axis = 1) # row-wise computation of the L2 norm
        H_tile_1_tile_norm_gpu = cp.asarray(H_tile_1_tile_norm)
        for k in range(0, H_tile_2.shape[0], tau):
            tmp = A_blocks[block_idx]
            AHHTnorm = cp.sparse.csr_matrix(tmp)
            AHHTnorm.data[:] = 0
            H_tile_2_tile = cp.asarray(H_tile_2[k:min(k + tau, H_tile_2.shape[0]), :])
            H_tile_2_tile_norm = np.linalg.norm(H_tile_2_tile, 2, axis = 1) # row-wise computation of the L2 norm
            H_tile_2_tile_norm_gpu = cp.asarray(H_tile_2_tile_norm)
            # kernels.ahhtnorm[min(65535, tmp.shape[0]), 128](AHHTnorm.data, AHHTnorm.indices, AHHTnorm.indptr, H_tile_1_tile, H_tile_2_tile,
            #                                                 H_tile_1_tile_norm_gpu, H_tile_2_tile_norm_gpu)
            kernels.ahhtnorm_shfl[min(65535, tmp.shape[0]), 128](AHHTnorm.data, AHHTnorm.indices, AHHTnorm.indptr, H_tile_1_tile, H_tile_2_tile,
                                                                 H_tile_1_tile_norm_gpu, H_tile_2_tile_norm_gpu)
            out_tile += AHHTnorm @ (H_tile_2_tile @ W_gpu)
            block_idx += 1
...
def cpu_computation(A: sparse.csr_matrix, H_tile_1: np.ndarray, H_tile_2: np.ndarray, W: List[np.ndarray],
                    bcast_comm: MPI.Cartcomm, reduce_comm: MPI.Cartcomm, layers: int, root: int) -> np.ndarray:
    ...
    for i in range(layers):
        out = layer_cpu(A, out, tmp, W[i], reduce_comm)
        if i < layers - 1:
            if bcast_rank == root:
                tmp = out
...
def gpu_computation(A: sparse.csr_matrix, A_blocks: List[sparse.csr_matrix], H_tile_1: np.ndarray,
    ...
    :param reduce_comm: The reduce communicator.
    :param layers: The number of layers for AGNN inference.
    :param root: The row of the process grid.
    :return: The output (new H matrix tile after the last layer).
    """
    ...
    bcast_rank = bcast_comm.Get_rank()
    for i in range(layers):
        out = layer_gpu(A, A_blocks, out, tmp, W[i], tau, reduce_comm)
        if i < layers - 1:
            if bcast_rank == root:
                tmp = out
            utils.bcast_matrix(tmp, bcast_comm, root)
...
if __name__ == "__main__":
    ...
    if bcast_rank == 0:
        H_tile_2 = utils.generate_dense_matrix(lNK, lNJ, dtype, rng)
    else:
    ...
    # ref = cpu_computation(lA, H_tile_1, H_tile_2, W, bcast_comm, reduce_comm, args['layers'], y)
    # utils.mpi_print(cart_rank, "Computing GPU output...")
    # val_gpu = gpu_computation(lA, A_blocks, H_tile_1, H_tile_2, W, tau, bcast_comm, reduce_comm, args['layers'], y)
    ...
    # cpu_runtimes = repeat("cpu_computation(lA, H_tile_1, H_tile_2, W, bcast_comm, reduce_comm, args['layers'], y); cart_comm.Barrier()",
    #                       repeat=1,
    #                       number=1,
    #                       globals={
    #                           **locals(),
    ...
    gpu_stmt = "gpu_computation(lA, A_blocks, H_tile_1, H_tile_2, W, tau, bcast_comm, reduce_comm, args['layers'], y); cp.cuda.get_current_stream().synchronize(); cart_comm.Barrier()"
