...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', '--layers', type=int, nargs="?", default=2, help='The number of layers in the GNN model.')
    parser.add_argument('--repeat', type=int, nargs="?", default=2, help='The number of times to repeat the benchmark.')
    ...
    utils.mpi_print(cart_rank, f'Testing {num_layers}-layer distributed {task} on {Px}x{Py} processes...')
    ...
    if model_name == 'GAT':
        model = gat_model_distr.GatModelDistr([NJ,]*num_layers, NL, lA_shape, lNI, tau, True, bcast_comm, reduce_comm, cart_comm, num_layers, inference_only)
        for layer in model.layers:
            layer.force_set_parameters(not inference_only, W=W_local, a_l=a_l_local, a_r=a_r_local)
    elif model_name == 'AGNN':
        model = agnn_model_distr.AGNNmodelDistr([NJ,]*num_layers, NL, lA_shape, lNI, tau, True, bcast_comm, reduce_comm, cart_comm, num_layers, inference_only)
        for layer in model.layers:
            layer.force_set_parameters(not inference_only, W=W_local)
    else:
        model = va_model_distr.VAmodelDistr([NJ,]*num_layers, NL, lA_shape, lNI, tau, True, bcast_comm, reduce_comm, cart_comm, num_layers, inference_only)
        for layer in model.layers:
            layer.force_set_parameters(not inference_only, W=W_local)
    ...
    # Logging the results
    filename = 'comm_comp_results.csv'

    if x == 0 and y == 1:
        # do not use diagonal processes to write to the file
        timing_data = model.layers[1].timing_data[-4:]
        with open(filename, 'a') as f:
            # modelname, num_nodes, dtype, Vertices, Edges, num_layers, feature_dim, forward comp, forward comm, backward comp, backward comm
            f.write(f'{model_name}\t{world_size}\t{num_vertices}\t{num_edges}\t{NJ}\t{timing_data[0]}\t{timing_data[1]}\t{timing_data[2]}\t{timing_data[3]}\n')
