...
def layer_gpu(A: sparse.csr_matrix, A_blocks: List[sparse.csr_matrix], H: np.ndarray, W: np.ndarray,
              tau: int) -> np.ndarray:
    ...
    for i in range(0, A.shape[0], tau):
        ...
        H_tile_1_norm = np.linalg.norm(H_tile_1, 2, axis = 1) # row-wise computation of the L2 norm
        ...
        for k in range(0, H.shape[0], tau):
            ...
            H_tile_2_norm = np.linalg.norm(H_tile_2, 2, axis = 1) # row-wise computation of the L2 norm
            H_tile_2_norm_gpu = cp.asarray(H_tile_2_norm)
            # kernels.ahhtnorm[min(65535, tmp.shape[0]), 128](AHHTnorm.data, AHHTnorm.indices, AHHTnorm.indptr, H_tile_1, H_tile_2,
            #                                                 H_tile_1_norm_gpu, H_tile_2_norm_gpu)
            kernels.ahhtnorm_shfl[min(65535, tmp.shape[0]), 128](AHHTnorm.data, AHHTnorm.indices, AHHTnorm.indptr, H_tile_1, H_tile_2,
                                                                 H_tile_1_norm_gpu, H_tile_2_norm_gpu)
...
def cpu_computation(A: sparse.csr_matrix, H: np.ndarray, W: List[np.ndarray], layers: int) -> np.ndarray:
    ...
    for i in range(layers):
        H_norm = np.linalg.norm(out, 2, axis = 1) # row-wise computation of the L2 norm
        H_norm = H_norm.reshape(H_norm.shape[0], 1)
        out = np.maximum(A.multiply(np.divide(out @ out.T, H_norm @ H_norm.T)) @ (out @ W[i]), 0)

    return out


def gpu_computation(A: sparse.csr_matrix, A_blocks: List[sparse.csr_matrix], H: np.ndarray, W: List[np.ndarray],
    ...
    out = H.copy()
    for i in range(layers):
        out = layer_gpu(A, A_blocks, out, W[i], tau)
