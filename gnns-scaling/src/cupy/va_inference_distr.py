...
def cpu_computation(A: sparse.csr_matrix, H_tile_1_in: np.ndarray, H_tile_2_in: np.ndarray, W: List[np.ndarray],
    ...
    :param H_tile_1_in, H_tile_2_in: Initial tiles of the H matrix for the first layer.
    ...
    H_tile_1 = H_tile_1_in.copy()
    H_tile_2 = H_tile_2_in.copy()
    bcast_rank = bcast_comm.Get_rank()
    for i in range(layers):
        tmp = A.multiply(H_tile_1 @ H_tile_2.T) @ (H_tile_2 @ W[i])
        reduce_comm.Allreduce(MPI.IN_PLACE, tmp, op=MPI.SUM)
        H_tile_1 = np.maximum(tmp, 0)

        # redistribution of H_tile_2
        if i < layers - 1:
            if bcast_rank == root:
                H_tile_2 = H_tile_1.copy()
            utils.bcast_matrix(H_tile_2, bcast_comm, root)

    return H_tile_1


def gpu_computation(A: sparse.csr_matrix, A_blocks: List[sparse.csr_matrix], H_tile_1_in: np.ndarray,
                    H_tile_2_in: np.ndarray, W: List[np.ndarray], tau: int, bcast_comm: MPI.Cartcomm,
    ...
    for l in range(layers):
        ...
        for i in range(0, A.shape[0], tau):
            ...
            for k in range(0, H_tile_2.shape[0], tau):
                ...
                AHHT.data[:] = 0
                # kernels.ahht[min(65535, tmp.shape[0]), 128](AHHT.data, AHHT.indices, AHHT.indptr, H_tile_1_tile, H_tile_2_tile)
                kernels.ahht_shfl[min(65535, tmp.shape[0]), 128](AHHT.data, AHHT.indices, AHHT.indptr, H_tile_1_tile, H_tile_2_tile)
                out_tile += AHHT @ (H_tile_2_tile @ W_gpu)
                block_idx += 1
...
if __name__ == "__main__":
    ...
    gpu_runtimes = repeat(gpu_stmt, setup=gpu_setup, repeat=num_repeats+num_warmup, number=1, globals={**locals(), **globals()})
    utils.mpi_print(cart_rank,
