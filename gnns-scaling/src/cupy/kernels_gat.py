...
def test_gat_forward_kernels():
    ...
    forward_lr_E_rowmax_kernel((min(65535,
                                    len(A.indptr) - 1), ), (128, ),
    ...
    gat_forward_softmax_1[min(65535,
                              len(A.indptr) - 1), 128](Alpha_dev, row_sum_dev, E_data_dev, cp.asarray(A.indices),
                                                       cp.asarray(A.indptr), row_max_dev)
    ...
    gat_forward_softmax_2[min(65535,
                              len(A.indptr) - 1), 128](Alpha_dev, row_sum_dev, cp.asarray(A.indices),
                                                       cp.asarray(A.indptr))
...
@cpx.jit.rawkernel()
def gat_backward_dZ_dAlpha_rowdot(out_data, row_dot, Alpha_data, dZ, M, indices, indptr):
...
@cpx.jit.rawkernel()
def gat_backward_softmax_leakyrelu_lr(dl, dr, l, r, Alpha_data, dAlpha_data, indices, indptr, row_dot):
...
def test_gat_backward_kernels():
    ...
    Alpha_data = utils.generate_dense_matrix(A.data.shape[0], 1, np.float32, rng).squeeze()
    ...
    gat_backward_dZ_dAlpha_rowdot[min(65535,
                                      len(A.indptr) - 1), 128](dAlpha_data_dev, row_dot_dev, cp.asarray(Alpha_data),
                                                               cp.asarray(dZ), cp.asarray(M), cp.asarray(A.indices),
                                                               cp.asarray(A.indptr))
    ...
    for i in range(len(A.indptr) - 1):
        for j in range(A.indptr[i], A.indptr[i + 1]):
            rowNo = i
            colNo = A.indices[j]
            dE_data_ref[j] = (dAlpha_data_ref[j] - np.dot(dAlpha_data_ref[A.indptr[i]: A.indptr[i + 1]], Alpha_data[A.indptr[i]: A.indptr[i + 1]])) * Alpha_data[j]
            D_recomp = l[rowNo] + r[colNo]
            if D_recomp <= 0:
                dE_data_ref[j] *= 0.2
            dl_ref[rowNo] += dE_data_ref[j]
            dr_ref[colNo] += dE_data_ref[j]
    gat_backward_softmax_leakyrelu_lr[min(65535, len(A.indptr) - 1), 128](dl_dev, dr_dev, cp.asarray(l), cp.asarray(r), cp.asarray(Alpha_data), dAlpha_data_dev, cp.asarray(A.indices), cp.asarray(A.indptr), row_dot_dev)
